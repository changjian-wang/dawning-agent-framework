...
3. 同一主题最多一处权威页（`canonical: true`）；主题已有 active 内容时必须有且仅有一处。
4. 所有事实性判断可追溯到 `raw/`。
5. 优先更新已有页面，而不是持续新建。
...
- 禁止二级及更深子目录。
- 未经修改本 SCHEMA，不得新增 `docs/` 顶层目录或 `pages/` 下的 type 目录（红线见 §10.3 #9/#10）。
...
1. 这是对所有页面持续生效、违反即错误的硬约束（命名规范、提交流程、风格守则、API 契约）？→ `rule`
...
| 字段 | 指向范围 | 额外规则 |
|---|---|---|
| `supersedes` | 同 type 旧页（hub→hub、entity→entity、concept→concept、comparison→comparison、rule→rule、ADR→ADR） | 取代关系图必须为 DAG（无环）；被指向方应在 §4.6 下转入 archived/superseded |
| `part_of` | hub 或明确父 concept / rule；元素目标页 `type` 必须 ∈ {hub, concept, rule}（entity / comparison / adr 不得作为父页） | 数组非空时首元素必须是 hub；其余可为父 concept / rule。数组为 `[]` 仅允许于（1）root hub，或（2）`supersedes` 非空的取代页（可达性由 §4.6.3 满足） |
...
| `stale` | `active`, `archived` | 需复查；`updated` 距今 > 30 天须把 `canonical` 降为 `false`（页面保持 `stale`）或将 `status` 转为 `archived`（见 §4.6.2） |
...
- `canonical: true` 表示同主题当前权威页；同主题最多一页为 `true`，且当主题存在 `status: active` 的页面时必须有且仅有一处 `canonical: true`（与 §1 设计原则 #3 一致）。
- `draft` 必须 `canonical: false`。
- `archived` 必须 `canonical: false`，并填写非空 `archived_reason`。
- `stale` 暂时允许 `canonical: true`；以 `updated` 为度量起点（进入 `stale` 时必须把 `updated` 刷新为当日），距今超过 30 天仍未复查必须把 `canonical` 降为 `false`（页面保持 `stale`）或将 `status` 转为 `archived`。
...
本 SCHEMA 中「主入口」指以下两种路径之一：

1. hub 页「从这里开始」 H2 中的链接；
2. 任意页面 front matter `part_of` 指向。

`related` 、`supersedes` 反向追溯、以及正文说明性链接不算主入口。§8.3.1 中出现的「主入口」一律指上述两种。
...
- `相关页面` 章节列出本页 `related / part_of / supersedes` 中的链接。
- `来源` 章节列出本页 `sources` 中的链接；`sources` 非空时必须出现 `来源` H2，即便模板未将其列为必备。
- 正文 markdown 链接使用相对当前文件的路径（如 `../../PURPOSE.md`）；front matter 链接字段仍按 §4.5 以相对 `docs/` 的路径书写（如 `pages/adrs/foo.md`）。二者联合表达同一页面时必须一致。

### 6.1 Hub

> 必备 H2：`范围`、`从这里开始`、`相关页面`。可选 H2：`不在范围内`、`阅读地图`、`来源`（`sources` 非空时必须出现）。
...
> 必备 H2：`规则`、`正例`、`反例`、`相关页面`。可选 H2：`例外`、`来源`（`sources` 非空时必须出现）。
...
> 必备 H2：`背景`、`备选方案`、`被否决方案与理由`、`决策`、`影响`、`复议触发条件`、`相关页面`。可选 H2：`来源`（`sources` 非空时必须出现）。
...
- `canonical` 同主题至多一页 `true`；同主题存在 active 页时必须且仅有一处 `canonical: true`。
- `status: archived` / `adr_status: superseded` 必须 `canonical: false`。
- `draft` 页面是否 `canonical: false`。
- `status: archived` 是否有非空 `archived_reason`。
- `status: archived` 不得作为任何 hub / 其它页的主入口被链接（历史 `related / supersedes` 反向追溯除外）。
- `status` 转换是否符合 §4.6.1 矩阵；`archived` 不得回到其它状态。
- `stale` 且 `canonical: true` 时，`updated` 距今 > 30 天须降级为 `canonical: false` 或转入 `archived`。
...
- 不得存在第二个 `type: hub` 且 `part_of: []` 的页面（root hub 唯一）。
...
- 正文 markdown 链接未使用相对当前文件的路径（§6.0），或与同页 front matter 链接指向不一致。
...
1. 新建一份与旧页同 `type` 的新页，按 §6 对应模板起草（任意 type 均可，常见为 ADR / rule，亦适用于 entity / concept / comparison / hub）。
2. 在新页 front matter `supersedes` 中加入旧页路径（§4.5 字段专属规则；元素必须与本页 `type` 一致）。
3. ADR 把 `adr_status` 改为 `superseded`；其它 type 把 `status` 改为 `archived`。ADR 亦可走 `archived`（§4.6.1 状态机允许），但优先 supersede；无论哪种，旧页必须设 `canonical: false`。
...
16. 不创建第二个 `part_of: []` 的 hub；root hub 唯一且固定为 `pages/hubs/agent-os.md`。

## 11. 派生物

以下文件不是启动前提（与 §1 #6 一致），由脚本在自动化就绪后生成；自动化前不存在也合法，agent 不得手维（见 §10.1 #3）：
...
| 3.28 | 2026-04-27 | 深度审计修复：§2.1 修正 §10 引用断链；§1 #3 与 §4.6.2 措辞统一为「最多一处 / 有且仅有」；§4.6.2 stale 30 天度量起点定为 `updated`；§6.0 与 hub/rule/ADR 模板补可选「来源」H2 并加 `sources` 非空必须出现规则；§4.5 `supersedes` 同 type 范围扩展到全 6 类；§3.1 #1 措辞具体化；§10.4 #16 改为机器可校（不得有第二个 `part_of: []` 的 hub）；§8.3.1 拆分 archived 双重判定 + 增同 type supersedes / 来源 H2 / root hub 唯一性；§8.3.2 删除与阻断重复条；§11 显式声明派生物降级条款；PURPOSE §5 同步 `front matter` 术语 |
