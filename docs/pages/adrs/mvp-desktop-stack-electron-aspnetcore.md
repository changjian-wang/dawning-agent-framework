...
用户明确倾向成熟框架，并提出可使用 ASP.NET Core 做后端。后续又明确：第一版需要支持 Windows / macOS，默认接入 GPT / DeepSeek，前端使用 Arco Design Vue + TypeScript + Vite；后端应遵循 DDD 分层骨架，并采用 DI、Options、Dapper、Repository / UnitOfWork 等成熟工程模式；数据存储按本地应用数据目录 + SQLite 的建议执行，且实现永远方案先行，不一次性生成目录或代码骨架。

Dawning.ORM.Dapper 已包含 SQLiteAdapter，但 SDK 本身不携带 SQLite 驱动包；应用项目需要自行引用 Microsoft.Data.Sqlite 并创建 SqliteConnection。因此，第一版必须用最小 SQLite CRUD 测试验证 adapter 命中与基本分页 / CRUD 行为，再扩大数据层实现。
...
- 前端实现：Arco Design Vue + TypeScript + Vite、React + TypeScript + Vite、直接套用完整后台管理模板。
...
- 当前前端方向已收敛到 Vue 3、Arco Design Vue、router、store、typed api client 和桌面 layout 组织，另起 React 栈会增加认知成本。

**直接套用完整后台管理模板**：

- 可以快速得到后台管理壳，但会把个人桌面助手拖向 admin 产品形态。
- Dawning Agent OS 第一版只需要 chat、inbox、Memory Ledger 等轻量界面，应采用面向桌面助手的目录组织、API 封装、Arco 接入和工程习惯，而不是复制完整权限、菜单、后台布局复杂度。
...
- MySQL 适合网关 / 身份这类服务端系统，但本项目第一版是本地桌面产品。
...
- 前端 UI 使用 Arco Design Vue + TypeScript + Vite；采用桌面产品所需的 router、store、typed api client 和轻量 layout 组织，不照搬后台管理模板。
- ASP.NET Core 负责本地 agent runtime：LLM provider、agent orchestration、Memory Ledger、inbox、兴趣权重、动作分级、操作记录和本地 API。
- 后端采用 DDD 分层骨架：Api 负责本地 HTTP 边界，Application 负责用例编排与 DTO，Domain 负责实体、值对象、领域事件、仓储接口与产品权限概念，Domain.Services 负责跨实体 / 值对象的纯领域服务与领域策略，Infrastructure 负责 SQLite / Dapper、LLM provider、系统路径、外部适配与 DI 注册。
- 后端采用 DI、Options、Dapper、Repository / UnitOfWork 等成熟工程模式，但第一版不引入 OpenIddict、Kafka、Redis、YARP、完整权限系统等服务端复杂度。
- 后端采用严格依赖边界：Dapper、SQLite、HttpContext、YARP、MailKit、MediatR 等具体技术不得穿透进 Domain / Domain.Services；Application 只依赖端口和必要的 .NET abstractions。
...
- `src/Dawning.AgentOS.Api/`：ASP.NET Core 本地 API 宿主。
- `src/Dawning.AgentOS.Application/`：用例编排、DTO、应用服务接口与命令 / 查询边界。
- `src/Dawning.AgentOS.Domain/`：实体、值对象、领域事件、仓储接口与动作分级 / 权限概念。
- `src/Dawning.AgentOS.Domain.Services/`：纯领域服务与领域策略，只依赖 Domain，不承载应用编排或基础设施访问。
- `src/Dawning.AgentOS.Infrastructure/`：SQLite / Dapper、LLM provider、系统路径、外部适配与 DI 注册。
- `tests/`：后端 Architecture / Domain / Domain.Services / Application / Infrastructure / API 测试。
...
- Arco Design Vue + TypeScript + Vite 成熟且适合快速交付，降低第一版 UI 工程不确定性。
- ASP.NET Core 后端与未来 framework 抽取方向兼容，核心逻辑可先以产品代码存在。
- DDD 分层会在第一刀固定依赖方向，避免后续 Memory、inbox、LLM provider、权限边界扩展时再大规模拆层。
...
- DDD 骨架会比扁平后端多几个项目；第一版必须克制，只放边界、接口和最小验证用例，不提前实现完整 enterprise 模块。
