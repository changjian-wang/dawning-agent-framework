...
subtype: product
canonical: true
summary: 模糊指令先关联上下文推断；能推断则给候选或按动作级别处理，推断不足才询问。
...
> 模糊指令先关联上下文推断；能推断则给候选或按动作级别处理，推断不足才询问。

## 背景

dawning-agent-os 的核心交互原则是 options-over-elaboration：user 不需要把需求写清楚，agent 应主动给候选让 user 选择。但从人类协作视角看，模糊指令并不意味着立刻反问；更自然的顺序是先关联当前对话、当前任务、已授权材料与长期记忆来推断。只有推断不出来，或推断结果越过动作级别边界时，才询问 user。

## 备选方案

- 方案 A：agent 尽量猜测 user 意图并直接执行。
- 方案 B：模糊指令先做上下文推断；能推断则给候选或按 ADR-004 动作级别处理，推断不足才询问。
...
- 模糊指令默认先关联当前对话、当前任务、已授权材料与长期记忆，推断 user 最可能的意图。
- 能推断时，默认给 2–4 个候选方案，每个候选用一句话说明核心差异；不要先把开放问题还给 user。
- 推断不出来、候选差异无法收敛、或风险超过动作级别边界时，才询问 user；询问也优先给选择题，而不是开放问答。
...
- 保留「不用写 prompt」的核心体验，同时符合人类协作中「先看上下文再问」的直觉。
- 降低抽象指令带来的误操作风险。
- 与 ADR-004 的动作分级和 ADR-008 的主动性边界一致。

**代价 / 风险**：

- 上下文推断质量直接决定体验；推断过度会造成误解，推断不足会退化成普通问答。
