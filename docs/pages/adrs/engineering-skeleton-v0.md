...
canonical: false
summary: 拟议的第一刀工程骨架只验证 Electron 桌面壳、Arco Design Vue UI、DDD 分层 ASP.NET Core 本地后端、startup token 通信与 SQLite/Dapper 最小数据链路。
...
> 拟议的第一刀工程骨架只验证 Electron 桌面壳、Arco Design Vue UI、DDD 分层 ASP.NET Core 本地后端、startup token 通信与 SQLite/Dapper 最小数据链路。
...
本页是拟议方案。它记录下一步 scaffold 边界，但在 user 明确确认前不授权创建产品目录或代码。
...
本方案在进入 accepted 前由 user 确认。若确认前发现目录边界、进程模型、SQLite/Dapper 验证顺序或第一刀非目标需要调整，应直接修改本草案，而不是开始 scaffold。
