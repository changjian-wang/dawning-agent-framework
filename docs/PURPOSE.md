...
- **最小可用形态（MVP）**：**主场景 = 信息整理**（本地文件 / 笔记 / 邮件本地副本的归类、打标、检索），走完整闭环。选型理由：最能验证「长期记忆 + 选择题优先」差异化（零记忆不可用）、失败可逆、闭环最短、累积记忆数据快；详见 ADR-005。
- **副场景（侦察兵）**：
  - 日程：只做「读 + 候选生成」（识别冲突、给出推谁 / 合并 / 延期的 2–3 个候选），不调用日历写 API。
  - 生活决策：只列 tradeoff / 给 2–4 个候选，不下结论。
  副场景与主场景共享 Memory 模块；是否能被复用是验证「通用层抽取」是否成立的依据（对应 §通用层抽取「哪些代码该抽进 framework」）。
- **最终形态（北极星，非承诺）**：长期收敛于「个人计算的中间层 / 个人 OS」——长期记忆与意图理解作为系统级能力，其它 app 接入它而不是它去接入其它 app。本条仅作为路径过滤器（每个架构决策反问「是否朝此方向收敛」），不进入 §4.1 thesis、不立刻驱动功能清单；待 MVP 跑通且 Memory 模块出现外部依赖后，再考虑升级为 §4.1 + 起草 ADR。
- **每次决策必答的两个问题**：
...
- **教程内容**：how-to 类入门文档不进 wiki，进 README / 官方 docs。反例锥点：本 wiki 只收「为什么这么做」（决策与机制），不收「怎么操作」（步骤与命令行）；踩坑与修复思路归§3.1，不算教程。
...
- **ADR 优先级**：§4.1 当前列出的全部边界判断无须一次性补齐 ADR。下面 4 项是**产品哲学根基**，应作为首批 ADR 立刻起草；其余等真正被产品决策触发时再补：
  ...
  4. 重要性级别定义（前置 ADR）——对应 §4.1 产品红线 #2 + §4.2「重要性级别如何定义」；在该 ADR 落地前，§4.1 #2 仅为声明性红线，无法被机械检验。
  5. **MVP 主场景选型 = 信息整理**（ADR-005）——对应 §2 「最小可用形态」；被否决方案：日程（失败不可逆、外部依赖重）、生活决策（执行闭环薄、记忆积累慢）。
...
*Purpose 版本：1.6 | 最后更新：2026-04-27 | 与 SCHEMA.md 协同演化*
