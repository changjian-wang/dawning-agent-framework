...
key_constraint:
  ...
  mvp_desktop_stack: MVP 采用 Electron 桌面壳 + Arco Design Vue + TypeScript + Vite UI + ASP.NET Core 本地后端；后端遵循 DDD 分层骨架；支持 Windows / macOS；第一版通信 localhost + 随机端口 + 启动 token；存储默认 SQLite，落在系统用户应用数据目录；数据访问采用 Dawning.ORM.Dapper 并由应用项目引用 Microsoft.Data.Sqlite，MySQL 暂作为未来可选适配
...
- **MVP 技术形态**：第一版采用桌面 App + ASP.NET Core 本地后端，桌面壳选 Electron，UI 使用 Arco Design Vue + TypeScript + Vite，支持 Windows / macOS。Electron 负责窗口、托盘、快捷入口、聊天 / inbox / Memory Ledger UI；ASP.NET Core 负责本地 agent runtime、Memory、inbox、权限与动作分级，并采用 DDD 分层骨架降低后续扩展成本。默认接入 GPT / DeepSeek，数据存储使用系统用户应用数据目录下的 SQLite；数据访问采用 Dawning.ORM.Dapper，MVP 默认 SQLite，MySQL 作为未来可选适配。详见 [ADR-016](pages/adrs/mvp-desktop-stack-electron-aspnetcore.md)。
...
- **MVP 桌面技术栈采用 Electron + Arco Design Vue + ASP.NET Core 本地后端**。成熟框架优先；第一版支持 Windows / macOS；Electron 负责桌面壳，Arco Design Vue + TypeScript + Vite 负责交互界面，ASP.NET Core 作为本地 agent runtime；后端采用 Api / Application / Domain / Domain.Services / Infrastructure 的 DDD 分层骨架；默认接入 GPT / DeepSeek；SQLite 数据放在系统用户应用数据目录，数据访问采用 Dawning.ORM.Dapper + Microsoft.Data.Sqlite；第一版不默认做云后端、账号系统、同步服务或 MySQL 部署。详见 [ADR-016](pages/adrs/mvp-desktop-stack-electron-aspnetcore.md)。
...
*Purpose 版本：1.19 | 最后更新：2026-04-28 | 与 SCHEMA.md 协同演化*
