...
LLM 在每一步交替进行推理（Thought）和行动（Action），并观察结果（Observation）：

```
Thought: 用户想查北京天气，我需要调用天气 API
Action:  get_weather(city="北京")
Observation: 晴，25°C，湿度 40%
Thought: 已获取结果，可以回答
Action:  final_answer("北京现在晴天，25°C，湿度 40%")
```

| 特性 | 说明 |
|------|------|
| 推理方式 | 逐步推理，每步 Thought → Action → Observation |
| 规划深度 | 无显式规划，贪心决策 |
| 适用场景 | 工具调用、信息检索、问答 |
| 代表框架 | OpenAI Agents SDK、LangChain ReAct Agent |

**优点**：实现简单，LLM 原生 function calling 天然支持。
...
| MS Agent Framework | ReAct（`ChatCompletionAgent`） | `AgentGroupChat` 多 Agent 协作 |
