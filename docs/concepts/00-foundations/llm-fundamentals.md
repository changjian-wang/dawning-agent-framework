...
这要求框架提供 `MaxSteps`、`MaxCost` 限制和自动上下文压缩机制（→ [上下文管理](../02-context-memory/context-management.md)）。
...
上下文管理策略的深入分析 → [上下文管理](../02-context-memory/context-management.md)。
...
| Token 限制 | `IConversationMemory`（→ [上下文管理](../02-context-memory/context-management.md)）|
...
- [上下文管理](../02-context-memory/context-management.md) — 五种上下文管理流派对比 + Dawning 双层记忆架构设计
