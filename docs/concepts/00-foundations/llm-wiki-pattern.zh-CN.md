...
````mermaid
graph TB
    subgraph SCHEMA["Schema（模式定义）<br/>CLAUDE.md / AGENTS.md — 结构约定、工作流规则"]
    end
    subgraph WIKI["Wiki（编译产物）<br/>Markdown 文件集 — 摘要、实体页、概念页、综述<br/>index.md（内容目录）+ log.md（操作日志）"]
    end
    subgraph RAW["Raw Sources（原始资料）<br/>不可变的文章、论文、数据文件 — LLM 只读不写"]
    end
...
<!-- LLM Wiki → Dawning Agent OS 架构映射 -->
````mermaid
graph LR
...
- [上下文管理](../02-context-memory/context-management.md) — 五种上下文管理流派 + 双层记忆架构
- [Agent Loop](../01-agent-core/agent-loop.md) — ReAct / Plan-and-Execute / Reflexion 执行模式
